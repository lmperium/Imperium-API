...
class DatabaseTest(unittest.TestCase):
    ...
    def test_analyst_registration(self):
        ...
        response = self.clien().post('api/analysts', data=self.analyst_info)
        self.assertEqual(response.status_code, 200)
        self.asserIn('Analyst already Registered', str(response.data))

    def test_get_all_analysts(self):
        """Test API get all analysts."""
        response = self.client().get('api/analysts')
        self.assertIn('No analysts registered', str(response.data))
    ...
    def test_get_analyst(self):
        ...
        res_json = json.loads(response.data.encode('utf-8').replace("'", "\'"))

        # Test for specified analyst.
        response = self.client().get(f'api/analayst/{res_json["analyst_id"]}')
        self.assertEqual(response.status_code, 200)
        self.assertIn('John', str(response.data))
